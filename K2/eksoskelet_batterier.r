...
D = read.table("K2/eksoskelet_batterier.csv", header=TRUE, sep=";")

Driftstid = D$Driftstid
Batteritype = D$Batteritype

## a. Middelv�rdi og standardafvigelse af alle data
y_streg = mean(Driftstid)
std = sd(Driftstid)

## b. Middelv�rdi og standardafvigelse for hver batteritype
# Data kan opdeles for hver batteritype: 
y_streg_1 = mean(Driftstid[1:5])
# o.s.v.
# Alternativt kan man benytte R-funtionen aggregate: 
y_streg_type = aggregate(x = Driftstid,            # Hvilke data skal der beregnes p�?
